...
func (h *ConfigHandler) updateConfig(ctx *fasthttp.RequestCtx) {
	...
	// Build the update on an independent copy: the live config must not change
	// until the mutation commits, or a failed store write would leave memory
	// ahead of the database. A shallow copy is enough because every edit below
	// replaces a whole field; nothing mutates through the shared pointer or
	// slice fields.
	currentConfig := *h.store.ClientConfig
	if prepared.enabled {
		// PostgreSQL is authoritative for runtime configuration, so the
		// compare-and-set baseline must be the stored row, not this node's
	...
	applyFailed := false
	// Upstream's ReloadClientConfigFromConfigStore is the single MCP apply path:
	// it re-reads the committed row, calls UpdateToolManagerConfig, and re-times
	// the tool-sync checkers using the current minutes-based unit. Duplicating
	// that here would re-introduce the seconds-based unit this fork used before
	// upstream switched mcp_tool_sync_interval to minutes.
