...
AMD64_DIGEST=$(docker manifest inspect "${IMAGE}:${IMAGE_TAG}-amd64" | jq -er '.manifests[0].digest')
ARM64_DIGEST=$(docker manifest inspect "${IMAGE}:${IMAGE_TAG}-arm64" | jq -er '.manifests[0].digest')
