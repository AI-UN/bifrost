...
- [x] **Responses->Chat fallback on native Anthropic/Gemini wires** (folder 76, feature slug `native-wire-responses-chat`: `anthropic/claude-haiku-4-5` on `/anthropic/v1/messages` and `gemini/gemini-2.5-flash` on `:streamGenerateContent`, both streaming, both forced with the targeted `x-bf-compat: ["convert_responses_to_chat"]`. Core flags a downgraded Responses request and calls `ChatCompletionStream`, leaving the chat->Responses re-assembly to the provider; only `openai.HandleOpenAIChatCompletionStreaming` did it, so the native Anthropic and Gemini chat streams emitted raw chat chunks. The Anthropic route then rendered them through `ToAnthropicChatStreamResponse` (`content_block_delta` with no `message_start`/`content_block_start`/`content_block_stop`/`message_stop`) and the GenAI route, which has no chat stream converter at all, dropped every chunk after the 200 was committed and returned an empty stream. Both models are `mode: chat` with no `/v1/responses` in the datasheet, so `markForConversion` really fires rather than passing vacuously the way folder 19 did.)
