...
<Note>
By default, DB-backed `config.json` uses `source_of_truth: "split"`: unchanged file-backed rows preserve UI/API edits, while changed file-backed rows are applied on the next startup. Use `source_of_truth: "config.json"` for strict file authority. This fork also adds `source_of_truth: "config_store"` for synchronized PostgreSQL-backed multinode runtime configuration; that mode is not available in upstream Bifrost OSS. See [Source of Truth & Reconciliation](/deployment-guides/config-json/source-of-truth).
</Note>
...
```json
{
  ...
  "access_profiles": [
    ...
    },
    {
      ...
      "description": "Profile granting a Virtual MCP and a narrowed tool allowlist",
      "is_active": true,
      "tags": ["mcp", "test"],
      "virtual_mcps": [
        {
