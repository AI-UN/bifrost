...
| `source_of_truth` | string | Runtime configuration authority: `"split"`, `"config.json"`, or the fork-only PostgreSQL-backed `"config_store"` mode | [Source of Truth](/deployment-guides/config-json/source-of-truth) |
...
| `"config_store"` *(fork only)* | PostgreSQL is authoritative for runtime configuration; file runtime sections are ignored and multinode revision synchronization is enabled |
...
`"config_store"` requires an enabled PostgreSQL `config_store`. It is a fork-specific extension for active-active OSS multinode deployments and is not available in upstream Bifrost OSS.
...
Supported provider keys: `anthropic`, `azure`, `bedrock`, `bedrock_mantle`, `cerebras`, `cohere`, `deepseek`, `deepinfra`, `gemini`, `groq`, `mistral`, `ollama`, `opencode-go`, `opencode-zen`, `openai`, `parasail`, `perplexity`, `sgl`, `vertex`, `openrouter`, `elevenlabs`, `huggingface`, `nebius`, `xai`, `replicate`, `vllm`, `runway`, `runware`, `fireworks`, `sarvam`, `wafer`, `databricks`.
