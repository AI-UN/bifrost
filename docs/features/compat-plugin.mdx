...
The LiteLLM compatibility plugin provides three request-shape transformations:
...
3. **Responses-to-Chat Conversion** - Automatically converts Responses API requests to chat completion format for models that only support chat APIs
4. **Drop Unsupported Params** - Automatically drops unsupported parameters if the model doesn't support them
5. **Unsupported Parameter Conversion** - Automatically converts unsupported parameters to their supported equivalents

When any request-shape transformation is applied, responses include `extra_fields.converted_request_type: <transformed_request_type>`. If request parameters are dropped, the keys are added in `extra_fields.dropped_compat_plugin_params`.
...
## 3. Responses-to-Chat Conversion

Some providers and models expose chat completions upstream but do not implement native Responses endpoints. This lets a Responses-only client (for example Codex) reach them. LiteLLM compatibility mode handles this by:

1. Checking that the caller speaks the OpenAI Responses wire
2. Checking if the model supports Responses natively (using the model catalog)
3. If not supported, converting your Responses input to chat completion format
4. Calling the chat completions endpoint internally
5. Transforming the response or stream back to Responses format

<Note>
**Symmetric Fallback**: The conversion only happens when the model doesn't support Responses natively. If a model has native Responses support, Bifrost uses it directly without any conversion.
</Note>

<Warning>
**Scoped to the OpenAI Responses wire.** Anthropic Messages (`/anthropic/v1/messages`) and Gemini GenerateContent (`/genai/v1beta/models/{model}`) are represented internally as Responses requests, but their clients expect native Anthropic or Gemini stream events, not Responses events. This fallback is therefore skipped on those endpoints even when the model catalog lists the model as chat-only — those providers already serve their own protocol natively.

The per-request `x-bf-compat: ["convert_responses_to_chat"]` header overrides this scoping, because it is an explicit opt-in. Be aware that forcing it on a streaming Anthropic or Gemini endpoint produces a degraded stream: the Anthropic converter emits `content_block_delta` events without the surrounding `message_start` / `content_block_start` / `content_block_stop` / `message_stop` lifecycle, and the Gemini endpoint has no chat-chunk converter at all.
</Warning>
...
3. There you can enable the features you need:
   ...
   - **Convert Responses to Chat** - converts Responses API requests to chat completions for models that only support chat
   ...
   - **Azure DeepSeek** - converts Azure DeepSeek responses requests to chat completions so reasoning is preserved for coding harnesses
...
Namespace-scoped tools are no longer handled by this plugin. Bifrost core flattens them for every provider whose wire does not support the `namespace` tool type, renames each nested function to `<namespace>__<function>` so names stay unique, and maps returned `function_call` items back to the original `name` plus `namespace`.
...
Responses to chat completion conversion works with providers that support chat completions but lack native Responses support:

| Provider | Native Responses | With Fallback |
|----------|------------------|---------------|
| Groq | No | Yes |
| Cerebras | No | Yes |
| Ollama | No | Yes |
...
### Transformation 3: Responses-to-Chat Conversion

**Applies to:** Responses requests on chat-only models

| Phase | Original | Transformed |
|-------|----------|-------------|
| Request | Responses input with `role: "user"` | Chat message with `role: "user"` |
| Request | `responses` or `responses_stream` request type | `chat_completion` request type |

### Metadata Set on Transformed Responses

When any request-shape transformation is applied:
...
- When `x-bf-compat: ["<setting1>","<setting2>"]`, enables the specified settings (available settings: `convert_text_to_chat`, `convert_chat_to_responses`, `convert_responses_to_chat`, `should_drop_params`, `should_convert_params`, `azure_deepseek`)
...
- Maintaining backward compatibility with text completion, chat completion, or Responses interfaces
