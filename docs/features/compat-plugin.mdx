...
Some providers and models expose chat completions upstream but do not implement native Responses endpoints. LiteLLM compatibility mode automatically handles this by:

1. Checking if the model supports Responses natively (using the model catalog)
2. If not supported, converting your Responses input to chat completion format
3. Calling the chat completions endpoint internally
4. Transforming the response or stream back to Responses format
