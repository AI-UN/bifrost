...
- SiliconFlow Files support is limited to upload and list with `purpose=batch`; Batch support is limited to create, list, retrieve, and cancel. Batch results and file retrieve/delete/content are unsupported.
- SiliconFlow Responses are translated through Chat Completions. Stored response lifecycle operations and `previous_response_id` are unsupported.
