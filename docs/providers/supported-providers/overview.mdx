...
| Provider                             | Models | Text | Text (stream) | Chat | Chat (stream) | Responses | Responses (stream) | Images | Images (stream) | Image Edit | Image Edit (stream) | Image Variation | Embeddings | TTS | TTS (stream) | STT | STT (stream) | Files | Batch | Count tokens | Rerank | Decisions | OCR | Video | Video Remix | Containers | Passthrough | Passthrough (stream) |
| ------------------------------------ | ------ | ---- | ------------- | ---- | ------------- | --------- | ------------------ | ------ | --------------- | ---------- | ------------------- | --------------- | ---------- | --- | ------------ | --- | ------------ | ----- | ----- | ------------ | ------ | --------- | --- | ----- | ----------- | ---------- | ----------- | -------------------- |
...
| SiliconFlow CN (`siliconflow-cn/<model>`) | ✅     | ✅   | ✅            | ✅   | ✅            | ✅        | ✅                 | ✅     | ❌              | ✅         | ❌                  | ❌              | ✅         | ✅  | ✅           | ✅  | ❌           | ✅    | ✅    | ❌           | ✅     | ❌     | ❌  | ✅    | ❌          | ❌         | ❌          | ❌                   |
| SGL (`sgl/<model>`)                  | ✅     | ✅   | ✅            | ✅   | ✅            | ✅        | ✅                 | ❌     | ❌              | ❌         | ❌                  | ❌              | ✅         | ❌  | ❌           | ❌  | ❌           | ❌    | ❌    | ❌           | ❌     | ❌     | ❌  | ❌    | ❌          | ❌         | ❌          | ❌                   |
...
| Wafer (`wafer/<model>`)              | ✅     | ✅   | ✅            | ✅   | ✅            | ✅        | ✅                 | ❌     | ❌              | ❌         | ❌                  | ❌              | ❌         | ❌  | ❌           | ❌  | ❌           | ✅    | ❌    | ❌           | ❌     | ❌     | ❌  | ❌    | ❌          | ❌         | ❌          | ❌                   |
| xAI (`xai/<model>`)                  | ✅     | ✅   | ✅            | ✅   | ✅            | ✅        | ✅                 | ✅     | ❌              | ❌         | ❌                  | ❌              | ❌         | ❌  | ❌           | ❌  | ❌           | ❌    | ❌    | ❌           | ❌     | ❌     | ❌  | ❌    | ❌          | ❌         | ❌          | ❌                   |
| Z.AI (`zai/<model>`, international) | ❌     | ❌   | ❌            | ✅   | ✅            | 🟡        | 🟡                 | ✅     | ❌              | ❌         | ❌                  | ❌              | ❌         | ❌  | ❌           | ❌  | ❌           | ❌    | ❌    | ✅           | ❌     | ❌     | ❌  | ❌    | ❌          | ❌         | ❌          | ❌                   |
| Zhipu AI (`zhipu/<model>`, mainland China) | ❌     | ❌   | ❌            | ✅   | ✅            | 🟡        | 🟡                 | ✅     | ❌              | ❌         | ❌                  | ❌              | ✅         | ❌  | ❌           | ❌  | ❌           | ❌    | ❌    | ✅           | ✅     | ❌     | ❌  | ❌    | ❌          | ❌         | ❌          | ❌                   |

`zai` is Z.AI's international service, while `zhipu` is Zhipu AI's mainland China service. For these providers, Responses requests use Bifrost's Chat Completions mapping rather than a native Responses endpoint.
...
- SiliconFlow runs two independent services, modelled as two providers: `siliconflow` is the international service (api.siliconflow.com) and `siliconflow-cn` is the mainland China service (api.siliconflow.cn). They have separate accounts, API keys and model catalogues, and each provider defaults to its own origin — `network_config.base_url` does not switch regions.
- SiliconFlow Files support is limited to upload and list with `purpose=batch`; Batch support is limited to create, list, retrieve, and cancel. Batch results and file retrieve/delete/content are unsupported. This applies to both regions.
- SiliconFlow Responses are translated through Chat Completions. Stored response lifecycle operations and `previous_response_id` are unsupported. This applies to both regions.
- SiliconFlow CN text completions are forwarded to `/v1/completions`, which that service does not publish an API reference for; its FIM guide routes `prefix`/`suffix` through `/v1/chat/completions` instead. The operation is not blocked, but availability is not guaranteed.
...
For a streaming request, the provider sends many SSE frames rather than one body, so `raw_response` is the concatenation of all of them:

- Every SSE `data:` payload Bifrost read from the provider is included, in the order received.
- Payloads are joined by a blank line.
- The `data:` prefix itself is **not** included, and neither is the terminal `[DONE]` marker.

Frames that carry no forwardable content are included too. That matters most for the final usage frame: with `stream_options.include_usage`, OpenAI-compatible providers send the whole request's token counts on a last chunk whose `choices` array is empty. Bifrost bills from that frame, so keeping it in `raw_response` is what lets you reconcile a provider invoice against the normalized `usage` Bifrost reports.

A complete captured stream therefore looks like this - the opening role-only frame, the content frames, the finish-only frame, and the usage-only frame:

```text
{"id":"chatcmpl-123","object":"chat.completion.chunk","choices":[{"index":0,"delta":{"role":"assistant"},"finish_reason":null}],"usage":null}

{"id":"chatcmpl-123","object":"chat.completion.chunk","choices":[{"index":0,"delta":{"content":"Hello"},"finish_reason":null}],"usage":null}

{"id":"chatcmpl-123","object":"chat.completion.chunk","choices":[{"index":0,"delta":{},"finish_reason":"stop"}],"usage":null}

{"id":"chatcmpl-123","object":"chat.completion.chunk","choices":[],"usage":{"prompt_tokens":1000,"completion_tokens":100,"total_tokens":1100}}
```

A single streamed chunk's `extra_fields.raw_response` may hold more than one frame, since frames that produce no chunk of their own are carried by the next chunk that is sent - or, for the trailing finish and usage frames, by the terminal chunk. Parse the value as blank-line-separated payloads rather than as a single JSON object.

<Note>
  If a stream is interrupted, the provider may never send its usage frame. In that case it is absent from `raw_response` for the same reason it is absent from the wire.
</Note>
